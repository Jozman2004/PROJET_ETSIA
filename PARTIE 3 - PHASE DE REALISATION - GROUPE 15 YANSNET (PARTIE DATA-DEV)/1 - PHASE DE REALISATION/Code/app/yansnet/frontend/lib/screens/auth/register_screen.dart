...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  final _otherResidenceCtrl = TextEditingController();
  bool _loading = false;
  bool _showPass = false;
  String? _selectedFiliere;
  String? _selectedResidence;
  bool _isOtherResidence = false;

  static const filieres = [
    'Génie Informatique',
    'Ingénieur Généraliste',
    'Ingénieur Généraliste par Apprentissage',
  ...
  Future<void> _register() async {
    ...
    if (_isOtherResidence) {
      ...
      if (residenceValue.isEmpty) {
        _showMsg('Veuillez saisir le nom de votre résidence', isError: true);
        return;
      }
    } else {
      residenceValue = _selectedResidence;
    }
    ...
    try {
      ...
      // Ajouter la nouvelle résidence à la liste si elle est saisie via "Autre" et n'existe pas encore
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: [
                ..._residences.map((r) => DropdownMenuItem(value: r, child: Text(r))),
                const DropdownMenuItem(value: 'Autre', child: Text('Autre (saisir)')),
              ],
              onChanged: (v) {
                setState(() {
                  _selectedResidence = v;
                  _isOtherResidence = (v == 'Autre');
                  if (!_isOtherResidence) {
                    _otherResidenceCtrl.clear();
                  }
                });
              },
            ),
            if (_isOtherResidence) ...[
              const SizedBox(height: 14),
              TextField(
                controller: _otherResidenceCtrl,
                decoration: InputDecoration(
                  labelText: 'Précisez votre résidence',
