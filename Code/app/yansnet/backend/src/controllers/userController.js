...
// ============================================================
// POSTS D'UN UTILISATEUR (avec reposts)
// ============================================================
...
// ============================================================
// STATUT DE FOLLOW (individuel et batch)
// ============================================================
...
exports.getFollowStatusBatch = async (req, res) => {
  const { ids } = req.query;
  if (!ids) return res.status(400).json({ error: 'ids manquants' });
  ...
  try {
    ...
    for (const targetId of userIds) {
      const followResult = await pool.query(
        `SELECT
          (SELECT COUNT(*) FROM follows WHERE follower_id = $1 AND following_id = $2) > 0 AS user_follows_target,
          (SELECT COUNT(*) FROM follows WHERE follower_id = $2 AND following_id = $1) > 0 AS target_follows_user`,
        [currentUserId, targetId]
      );
      const row = followResult.rows[0];
      result[targetId] = {
        userFollowsTarget: row.user_follows_target,
        targetFollowsUser: row.target_follows_user,
        isMutual: row.user_follows_target && row.target_follows_user,
      };
    }
    res.json(result);
  } catch (err) {
    console.error('getFollowStatusBatch error:', err);
    res.status(500).json({ error: 'Erreur serveur' });
  }
};
...
exports.followUser = async (req, res) => {
  ...
  try {
    ...
    await notify(
      userId,
      'new_follower',
      `${follower.rows[0]?.full_name || 'Quelqu\'un'} a commencé à vous suivre.`,
      followerId,
      'user'
    );
...
exports.blockUser = async (req, res) => {
  ...
  if (blockerId === userId) return res.status(400).json({ error: 'Vous ne pouvez pas vous bloquer vous-même' });
  try {
    const existing = await pool.query(
    ...
    // Supprime les éventuels follows mutuels
    await pool.query(
...
exports.unblockUser = async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Cet utilisateur n\'est pas bloqué' });
    }
...
exports.getBlockStatus = async (req, res) => {
  ...
  try {
    ...
    console.error('getBlockStatus:', err.message);
    res.status(500).json({ error: 'Erreur serveur' });
  }
};

// Export de la fonction utilitaire pour une utilisation éventuelle
