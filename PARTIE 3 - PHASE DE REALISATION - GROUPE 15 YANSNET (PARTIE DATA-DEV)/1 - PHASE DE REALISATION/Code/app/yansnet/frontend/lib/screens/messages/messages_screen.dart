...
class _MessagesScreenState extends State<MessagesScreen>
  ...
  // ✅ ID et username de l'utilisateur courant
  String _meId = '';
  String _meUsername = '';

  // Stocker les callbacks pour éviter les fuites et permettre la réattache
  void Function(Map<String, dynamic>)? _onNewMessageCallback;
  void Function(Map<String, dynamic>)? _onGroupMessageCallback;
  void Function(Map<String, dynamic>)? _onAddedToGroupCallback;
  void Function(Map<String, dynamic>)? _onGroupDeletedCallback;

  // Callback de reconnexion pour pouvoir le retirer
  ...
  void initState() {
    ...
    _meId = auth.user?.id ?? '';
    _meUsername = auth.user?.username ?? '';

    _tabController = TabController(length: 2, vsync: this);
    _loadAll();
    _listenSocketEvents();

    // Écouter la reconnexion pour recharger automatiquement
    _reconnectCallback = () {
      print('🔄 [messages_screen] Reconnexion détectée, rechargement des conversations');
  ...
  void dispose() {
    ...
    _socket.removeListener('group_message');
    _socket.removeListener('added_to_group');
    _socket.removeListener('group_deleted');
  ...
  void _listenSocketEvents() {
    _onAddedToGroupCallback = (_) => _loadGroupsSilently();
    _socket.onAddedToGroup(_onAddedToGroupCallback!);

    _onGroupDeletedCallback = (_) => _loadGroupsSilently();
    _socket.onGroupDeleted(_onGroupDeletedCallback!);

    _onGroupMessageCallback = (_) => _loadGroupsSilently();
    _socket.onGroupMessage(_onGroupMessageCallback!);

    _onNewMessageCallback = (_) => _loadConversationsSilently();
    _socket.onNewMessage(_onNewMessageCallback!);

    _socket.onMessageRead((_) => _loadConversationsSilently());
  }

  // ====================== CHARGEMENT =========================

  Future<void> _loadAll() async {
    await Future.wait([_loadConversations(), _loadGroups()]);
    _updateUnreadConversationsCount();
  }
  ...
  Future<void> _loadConversationsSilently() async {
    try {
      ...
      print('Erreur DM silencieux: $e');
    }
  }

  Future<void> _loadGroupsSilently() async {
    try {
      final data = await _api.getMyGroups();
      if (mounted) {
        setState(() {
      ...
      print('Erreur groupes silencieux: $e');
    }
  }

  // ====================== COMPTEURS ==========================

  int get _unreadDmCount => _conversations.where((c) =>
      (int.tryParse(c['unread_count']?.toString() ?? '0') ?? 0) > 0).length;

  int get _unreadGroupsCount => _groups.where((g) =>
      (int.tryParse(g['unread_count']?.toString() ?? '0') ?? 0) > 0).length;

  int get _totalUnreadConversations => _unreadDmCount + _unreadGroupsCount;

  void _updateUnreadConversationsCount() {
    _socket.setUnreadMessagesCount(_totalUnreadConversations);
  }

  // ====================== CRÉATION DE CONVERSATIONS / GROUPES ==========================
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return ListTile(
                      ...
                      onTap: () {
                        ...
                        ).then((_) => _loadConversationsSilently());
                      },
                    );
                  },
                ),
  ...
  Widget build(BuildContext context) {
    final unreadDm = _unreadDmCount;
    final unreadGroups = _unreadGroupsCount;
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: [
            Tab(text: 'Messages${unreadDm > 0 ? ' ($unreadDm)' : ''}'),
            Tab(text: 'Groupes${unreadGroups > 0 ? ' ($unreadGroups)' : ''}'),
          ],
        ),
      ),
      body: TabBarView(
  ...
  // ====================== TILES (avec MessageUtils et isSentByMe) ==========================
  ...
  Widget _buildDmTile(Map<String, dynamic> conv) {
    ...
    final lastSenderId = conv['last_sender_id']?.toString(); // fourni par le backend
    ...
    final bool isSentByMe = lastSenderId == _meId;

    String preview = MessageUtils.getPreviewText(lastMessage, isSentByMe: isSentByMe);
    if (lastMessage == null && lastFileType != null) {
      preview = _fileTypeLabel(lastFileType);
    } else if (lastMessage != null && lastFileType != null) {
      if (!MessageUtils.isPostShare(lastMessage)) {
        preview = '${_fileTypeLabel(lastFileType)} · $preview';
      }
    }
  ...
  Widget _buildGroupTile(Map<String, dynamic> group) {
    ...
    final lastSenderId = group['last_sender_id']?.toString();
    ...
    final bool isSentByMe = lastSenderId == _meId || lastSender == _meUsername;

    String preview = 'Aucun message';
    if (lastMessage != null || lastFileType != null) {
      final content = MessageUtils.getPreviewText(lastMessage, isSentByMe: isSentByMe);
      if (lastFileType != null && lastMessage != null && !MessageUtils.isPostShare(lastMessage)) {
        preview = lastSender != null
            ? '$lastSender: ${_fileTypeLabel(lastFileType)} · $content'
            : '${_fileTypeLabel(lastFileType)} · $content';
      } else if (lastMessage != null) {
        preview = lastSender != null ? '$lastSender: $content' : content;
      } else if (lastFileType != null) {
        preview = lastSender != null
            ? '$lastSender: ${_fileTypeLabel(lastFileType)}'
            : _fileTypeLabel(lastFileType);
      }
    }
...
class _GroupCreationScreenState extends State<GroupCreationScreen> {
  ...
  Map<String, dynamic> _mutualStatus = {};
  ...
  Future<void> _search(String query) async {
    ...
    try {
      ...
      final ids = results.map((u) => u['id'].toString()).toList();
      final statuses = await _api.getFollowStatusBatch(ids);
      setState(() {
        _searchResults = List<Map<String, dynamic>>.from(results)
            .where((u) => !_selected.any((s) => s['id'] == u['id']))
            .toList();
        _mutualStatus = statuses;
        _searching = false;
      });
    } catch (e) {
  ...
  void _toggle(Map<String, dynamic> user) {
    final status = _mutualStatus[user['id']];
    final isMutual = status != null && status['isMutual'] == true;
    if (!isMutual) return;
    setState(() {
                        ...
                        return ListTile(
                          leading: CircleAvatar(
                            ...
                            child: avatarUrl == null
                                ? Text(
                                    (user['full_name'] ?? '?')[0].toUpperCase(),
                                    style: TextStyle(color: isMutual ? Colors.white : Colors.grey),
                                  )
                                : null,
                          ),
                          title: Text(
                            user['full_name'] ?? '',
                            style: TextStyle(color: isMutual ? Colors.black : Colors.grey[600]),
                          ),
                          subtitle: Text(
                            '@${user['username'] ?? ''}',
                            style: TextStyle(color: isMutual ? Colors.grey[600] : Colors.grey[400]),
                          ),
                          trailing: isSelected
                              ? Icon(Icons.check_circle, color: isMutual ? const Color(0xFF9E1B22) : Colors.grey)
                              : Icon(Icons.add_circle_outline, color: isMutual ? const Color(0xFF9E1B22) : Colors.grey),
                          onTap: isMutual ? () => _toggle(user) : null,
                          enabled: isMutual,
                        );
                      },
                    ),
