...
import 'dart:async'; // ✅ indispensable pour StreamSubscription
...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  // ✅ Stocker la souscription pour l'annuler proprement
  ...
  void initState() {
    ...
    // ✅ S'abonner et stocker la souscription
    _notificationSubscription = _socket.onNewNotification((data) {
      print('📩 Notification reçue (brute) : $data'); // ✅ log de debug
      if (!mounted) return;
      setState(() {
        try {
          final notif = AppNotification.fromJson(data);
          _notifs.insert(0, notif);
          print('✅ Notification ajoutée : ${notif.content}');
        } catch (e) {
  ...
  Future<void> _load() async {
    try {
      ...
      final unread = data.where((json) => json['is_read'] == false).length;
      _socket.setUnreadCount(unread);
  ...
  Widget build(BuildContext context) {
    final unreadCount = _socket.unreadCount;
